...
public: 
  ...
  virtual void onInit() {
    ...
    gt_pub = nh.advertise<nav_msgs::Odometry>("/aftmapped_to_init", 16); // probably dont need this
  
    std::string topic_twist = private_nh.param<std::string>("topic_twist", "/eagle_data/twist"); 
    std::string topic_inlier_pc2 = private_nh.param<std::string>("topic_inlier_pc2", "/eagle_data/inlier_pc2"); // Inlier radar pointcloud
    std::string topic_outlier_pc2 = private_nh.param<std::string>("topic_outlier_pc2", "/eagle_data/outlier_pc2"); // Outlier radar pointcloud
    pub_twist = nh.advertise<geometry_msgs::TwistWithCovarianceStamped>(topic_twist, 5); // Radar ego velocity
...
private:
  ...
  void cloud_callback(const sensor_msgs::PointCloud::ConstPtr&  eagle_msg) { // const pcl::PointCloud<PointT>& src_cloud_r
    ...
    if (estimator.estimate(pc2_raw_msg, v_r, sigma_v_r, inlier_radar_msg, outlier_radar_msg))
    {
        ...
        twist.header.frame_id      = baselinkFrame; // added this frameid to the ego velocity twist topic
